...
Panel {
  ...
  // Touch root.dataRevision so the chart Repeater rebuilds whenever the
  // provider record refreshes — provider identity alone does not retrigger
  // the binding in Quickshell when the underlying record is mutated in place.
  readonly property var models: { void root.dataRevision; return modelRows(provider); }
          ...
          MouseArea {
            ...
            onClicked: function(mouse) {
              if (mouse.button === Qt.RightButton) {
                ...
                // Display-only policy (Josh 2026-08-31 10:08 PM EDT): no
                // click-to-setup dialogs for any provider. Click always
                // toggles the panel. Empty ring + kimiNeedsSetup/etc are
                // pure visual signals — no UI prompts, no fix buttons.
  ...
  KeyboardPanel {
      ...
      Flickable {
        ...
        Column {
          ...
          PanelHero {
            ...
            // OpenClaw shows its own version line inside the custom block
            // below; the PanelHero meta duplicates that, so suppress it.
            meta: root.provider && root.provider.providerId === "openclaw" ? "" : root.heroMeta(root.provider)
          ...
          Column {
            ...
            visible: !!root.provider && (
              root.provider.providerId === "openclaw" ||
              root.provider.providerId === "hermes" ||
              root.provider.providerId === "minimax" ||
              (root.provider.recentDays && root.provider.recentDays.length > 0)
            )
            width: parent.width
            spacing: Style.spacing.md
    ...
    readonly property bool isMiniMax: root.provider ? (root.provider.providerId === "minimax") : false
            ...
            // ---- Generic weekly chart + headline meter (for codex, claude, fireworks, etc.) ----
            Item {
              visible: !!root.provider &&
                root.provider.providerId !== "openclaw" &&
                root.provider.providerId !== "hermes" &&
          ...
          Column {
            id: modelSection
            // Hide the tokens-by-model chart entirely for OpenClaw — the
            // custom block above already covers what matters for that provider.
            visible: root.models.length > 0 && (!root.provider || root.provider.providerId !== "openclaw")
            width: parent.width
            spacing: Style.spacing.md
